...
int main() {
    ...
    while (!cities_to_visit.empty()) {
        ...
        if (city_prev_id < 0) {
            cities_min_distance[city_id] = 0;
        } else {
            cities_min_distance[city_id] = min(cities_min_distance[city_id], cities_min_distance[city_prev_id] + 1);
        }
        ...
        for (int i = 0; i < size(cities_min_distance); i++) {
            ...
            unsigned long distance_to_unreached_city = 0;
            distance_to_unreached_city += (unsigned long) abs(cities[i].first - cities[city_id].first);
            distance_to_unreached_city += (unsigned long) abs(cities[i].second - cities[city_id].second);
